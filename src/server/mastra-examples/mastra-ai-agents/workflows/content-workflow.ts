...
// AI-powered content analysis step
...
// This step validates text content and returns its type, word count, and validity.
const validateContentStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    if (!isValid) throw new Error(`Content too short: ${wordCount} words`);
    return { content: content.trim(), type, wordCount, isValid };
  },
});

// This step enhances validated content with metadata.
...
// This step generates a summary of the content.
...
// Move this export to the very end of the file after all step definitions and contentWorkflow export
